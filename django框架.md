...
外部函数，包裹内部函数，并将内部函数返回；内部函数为闭包

局部作用域的函数，可以将局部作用的数据进行暴露，而不是直接操作数据.
...
1. **应用场景**：
   - **数据封装**：通过闭包可以创建私有变量，这些变量对外部环境不可见，只能通过闭包函数进行访问和修改。
   - **模块化**：闭包可以用来创建模块，封装模块的内部状态和功能，对外只暴露有限的接口。
   - **回调函数**：在异步编程中，闭包可以用来传递回调函数，这些回调函数可以访问创建它们时的作用域中的变量。
2. **优点**：
   - 提供了一种强大的数据封装和信息隐藏机制。
   - 允许函数记住并访问其创建时的作用域。
3. **缺点**：
   - 可能会增加内存消耗，因为闭包会持有其词法环境中的变量，即使这些变量在函数外部已经不再需要。
...
- MVC 设计模式(其他语言的web框架)
    ...
    - MVC模式如图:
        ![](./images/mvc.png)
- MTV 模式（python主要框架django,flask,tornado）
...
- 火狐浏览器 可能通过开发者工具的 存储 -> Cookie
    ...
    - cookies 的内部的数据会在每次访问此网址时都会自动携带到服务器端，如果cookies过大会降低响应速度
...
- 缓存api
  ...
  #key: 字符串类型
  #value: Python对象
  #timeout：缓存存储时间（s）  默认值为settings.py CACHES对应配置的TIMEOUT
  #返回值：None
...
1，Last-Modified和If-Modified-Since
  ...
  Last-Modified: Fri, 22 Jul 2016 01:47:00 GMT
	...
	缺点：只能精确到秒，时间校验过期，容易发生单秒内多次修改，检测不到



2，ETag和If-None-Match

	Etag是服务器响应请求时，返回当前资源文件的一个唯一标识，hash值校验(由服务器生成)，只要资源有变化，Etag就会重新生成
...
- 中间件类:
    ...
    - 中间件类须实现下列五个方法中的一个或多个:
        ...
        - `def process_response(self, request, response):` 所有响应返回浏览器时被调用，在每个请求上调用，返回HttpResponse对象
        - `def process_exception(self, request, exception):` 当处理过程中抛出异常时调用，返回一个HttpResponse对象或None
        - `def process_template_response(self, request, response):` 在视图函数执行完毕且试图返回的对象中包含render方法时被调用；该方法需要返回实现了render方法的响应对象
    - 注： 中间件中的大多数方法在返回None时表示忽略当前操作进入下一个中间件事件，当返回HttpResponese对象时表示此次请求结束，返回给客户端 
...
class MyMiddleWare(MiddlewareMixin):
    def process_request(self, request):
        #返回HttpResponese时，直接从当前中间件的process_response开始执行
        print("中间件方法 process_request 被调用")

    def process_view(self, request, callback, callback_args, callback_kwargs):
        #callback:视图函数
        #callback_args：视图函数位置参数
        #返回HttpResponese时，从所有中间件的process_response开始执行
...
-  csrf_exempt去除某些不需要保护的

-  csrf_protect单独添加需要保护的
...
# Create your models here.
## 用自定义用户模型，替换默认用户模型，（提前迁移了user，需要删除迁移文件，删除数据库，重新迁移）
...
Django使用session和中间件将身份验证系统连接到请求对象中。

它们在每次请求中都会提供request.user属性。

如果当前用户没有登录会被设置为AnonymousUser,否则会被设置为User实例。

可以使用is_authenticated区分两者，例如：
...
如果一个已经验证的用户想要附加到当前的会话（session）中，需要login（）函数完成

它需要HttpRequest对象和User对象，通过Django的session框架，login（）会将用户id保存在session中。

注意：在匿名会话期间设置的任何数据，都会在用户登录后，保留在会话中。
...
装饰器，@login_required，限制访问视图

未登录会重定向到'settings.LOGIN_URL'
...
- 使用 `python manage.py runserver` 通常只在开发和测试环境中使用。runserver性能较低。
...
![](./images/Snipaste_2024-09-27_14-09-30.png)

## 返回json数据

Django框架负责作为api服务器，不再返回页面模板，而是返回Json数据
...
def add_book(request):
    response = {}
    try:
        # 数据库对象处理，一个对象就是数据库一条记录
        # Book.objects.create(book_name=request.GET.get('book_name'))
...
def show_books(request):
    ...
    try:
        ...
        # response['list'] =json.loads(books)
 		# locals()   //根据局部变量自动生成字典
        books = Book.objects.values('book_name') # 获取所有书籍
        response['list'] = [item['book_name'] for item in books]
        response['msg'] = 'success'
        response['error_num'] = 0
    except  Exception as e:
        response['msg'] = str(e)
...
如果出现跨域问题，需要在Django层注入header，用Django的第三方包`django-cors-headers`来解决跨域问题：
